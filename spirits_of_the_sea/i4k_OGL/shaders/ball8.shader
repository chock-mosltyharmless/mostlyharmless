...
{
   float sphere1 = length(rayPos - vec3(0.7, 1.9*sin(fTime0_X * 0.26), 0.0));
   float sphere2 = rayPos.y - 2.0;
   vec4 noiseAdd = noise(0.02 * rayPos.xzxz + 0.01 * rayPos.yxzy + vec4(10.0), 0.0, Texture0);
   float noise1 = noise(noiseAdd, 0.0, Texture0).r * 0.2;
   return sqrt(1.0 / (1.0 / (sphere1*sphere1+0.1) +  1.0 / (sphere2*sphere2+0.1))) - 1.3 + noise1;
}
...
vec3 getLightstream(vec3 pos, float coneSize, float fTime0_X)
{
   ...
   float noiseVal = abs(noiseData.r + noiseData.g + 0.3) + 0.0 + 0.0;
...
{  
   ...
   for (int step = 0; length(rayPos) < sceneSize && totalDensity < 0.99 && step < 100; step++)
   {
      ...
      if (implicitVal < 0.5 * coneSize)
      {
         ...
         vec3 refractRayDir = rayDir, refractRayPos = rayPos - implicitNormal * 0.01;
			  ...
			  refractStep++)
         {
             ...
             vec3 tightness = 10.0*vec3(0.28, 0.22, 0.15);
