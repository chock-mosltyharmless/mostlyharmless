...
vec2 getImplicit(vec3 rayPos, float fTime0_X)
{
   ...
   sphere1 += noise(rayPos.xzyx*0.08 + parameters[0][2] + vec4(fTime0_X * 0.02), 0., Texture0).r * 0.7;
   float sphere2 = 6.0 - length(rayPos+vec3(1.0, 0.0, 0.0));
   vec4 noiseAdd = 2.0 * noise(0.15 * rayPos.xzxz + 0.27 * rayPos.yxzy + vec4(0.01 * fTime0_X), 0.0, Texture0);
   float baseVal = sqrt(1.0 / (1.0 / (sphere1*sphere1+0.1) + 1.0 / (sphere2*sphere2+0.1)));// + noiseV1;
   baseVal += - 1.3 + length(noiseAdd.b) * 0.07;
   ...
   return vec2(baseVal + noiseV2 * smoothstep(0.0, 0.2, 0.2 - baseVal) * (0.25*noise2Amount), noise2Amount);
}
...
void main(void)
{
   ...
   vec3 camPos = vec3(-0.6, 0.5, -4.0 + 0.0 * sin(fTime0_X*0.5));
