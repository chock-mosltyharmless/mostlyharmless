...
static float getSD(float *position, int sdIndex)
{
	...
	if (sdIndex == METABALL_FIELD)
	{
		...
		value = (float)sqrt(1.0f / value) - 1.3f;
	}
...
static void updateSD(int sdIndex, float fDeltaTime)
{
	...
	for (int i = 0; i < 3*3; i++)
	{
		metaCenters[0][i] = 2.5f * (float)sin(randomValues[0][i][0]*time + 3.0f * randomValues[0][i][1]);
	}
}
...
void updateSwarmDestinations(int pathID, float fDeltaTime, float overShootAmount)
{
	...
	if (pathID == 0)
	{
		...
		for (int ptIdx = 0; ptIdx < numMPts; ptIdx++)
		{
			moveToPoint[ptIdx][0] = 3.04f * (float)sin(fCurTime * randomValues[ptIdx][0][0] + randomValues[ptIdx][0][1]) +
					2.71f * (float)sin(fCurTime * randomValues[ptIdx][0][2] + randomValues[ptIdx][0][3]);
			moveToPoint[ptIdx][1] = 2.53f * (float)sin(fCurTime * randomValues[ptIdx][1][0] + randomValues[ptIdx][1][1]) +
					3.22f * (float)sin(fCurTime * randomValues[ptIdx][1][2] + randomValues[ptIdx][1][3]);
			moveToPoint[ptIdx][2] = 2.57f * (float)sin(fCurTime * randomValues[ptIdx][2][0] + randomValues[ptIdx][2][1]) +
					2.83f * (float)sin(fCurTime * randomValues[ptIdx][2][2] + randomValues[ptIdx][2][3]);
		}
	...
	else if (pathID == 1)
	{
		...
		for (int triIdx = 0; triIdx < NUM_TRIANGLES; triIdx++)
		{
			...
			float phase = 10.0f * (float)sin(zPos*0.05f + fCurTime*0.3f) * 0.15f;
			float bestAngle = 0;
			int bestLine = 0;
			float closestDist = 1.0e20f;
			for (int i = 0; i < 4; i++)
			{
				float linex = (float)sin(phase);
				float liney = (float)cos(phase);
			...
			tris.moveToPoint[triIdx][0] = 2.7f * (float)sin(bestAngle);
			tris.moveToPoint[triIdx][1] = 2.7f * (float)cos(bestAngle);
			float direction = (float)sin(fCurTime*0.4f);
