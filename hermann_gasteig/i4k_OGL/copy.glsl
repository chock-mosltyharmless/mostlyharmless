...
{   
	...
	if (mode > 0.5)
	{
		gl_FragColor = 0.25 * texture2D(Texture0, 0.5*objectPosition.xy + 0.5 + 0.5 * stepSize);
		gl_FragColor += 0.25 * texture2D(Texture0, 0.5*objectPosition.xy + 0.5 + 0.5 * vec2(stepSize.x, -stepSize.y));
		gl_FragColor += 0.25 * texture2D(Texture0, 0.5*objectPosition.xy + 0.5 + 0.5 * vec2(-stepSize.x, stepSize.y));
		gl_FragColor += 0.25 * texture2D(Texture0, 0.5*objectPosition.xy + 0.5 - 0.5 * stepSize);
	}
	else
