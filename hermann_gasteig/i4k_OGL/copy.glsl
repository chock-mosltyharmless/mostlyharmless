...
{   
	...
	else
	{
		...
		vec2 startPos = 0.5 * objectPosition.xy + 0.5 - 4.5 * stepSize;
		gl_FragColor = vec4(0.0);
		for (int i = 0; i < 39; i++)
		{
			float localGain = 14.5 - abs(float(i)-14.5);
			gl_FragColor += localGain * gain * max(texture2D(Texture0, startPos) - vec4(reduction), vec4(0.0));
			startPos += stepSize;
		}
	}
}