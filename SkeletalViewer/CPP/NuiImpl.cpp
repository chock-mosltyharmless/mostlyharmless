...
void CSkeletalViewerApp::Nui_Snapshot( )
{
	// 5 seconds timer...
	m_Timer = 30*10;
}
...
void CSkeletalViewerApp::Nui_GotDepthAlert( )
{
    ...
    if( LockedRect.Pitch != 0 )
    {
		...
		if (m_Timer < 10 && m_Timer >= 0)
		{
			char filename[256];
			sprintf(filename, "snapshot/snapshot%d.sht", 10-m_Timer);
			...
			for( int y = 0 ; y < 240 ; y++ )
			{
				for( int x = 0 ; x < 320 ; x++ )
				{
					fprintf(fid, "%d", (int)(*pBufferRun));
					if (x < 319) fprintf(fid, " ");
					pBufferRun++;
				}
				fprintf(fid, "\n");
			}
#else
			//for (int t = 0; t < 320*240; t++)
			//{
				//pBuffer[t] >>= 3;
			//}
			fwrite(pBuffer, sizeof(USHORT), 320*240, fid);
#endif
			fclose(fid);
		}
...
RGBQUAD CSkeletalViewerApp::Nui_ShortToQuad_Depth( USHORT s )
{
    ...
    RGBQUAD q;
	int qrgbRed, qrgbGreen, qrgbBlue;
    qrgbRed = qrgbBlue = qrgbGreen = l;
	qrgbBlue = l/4;
	qrgbGreen = l/2;

	if (m_Timer > 0) qrgbBlue = qrgbGreen = 0;

	if (qrgbBlue > 255) qrgbBlue = 255;
	if (qrgbRed > 255) qrgbRed = 255;
	if (qrgbGreen > 255) qrgbGreen = 255;

	q.rgbBlue = qrgbBlue;
	q.rgbRed = qrgbRed;
	q.rgbGreen = qrgbGreen;
