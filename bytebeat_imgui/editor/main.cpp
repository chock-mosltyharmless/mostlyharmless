...
int WINAPI WinMain( HINSTANCE instance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
    ...
    while (!done) {
        ...
        // Tip: if we don't call ImGui::Begin()/ImGui::End() the widgets automatically appears in a window called "Debug".
        {
            ...
            // Here is the text input for the synthesis
            const int kMaxCodeSize = 1024;
            static char code[kMaxCodeSize + 1];
            ImGui::InputText("ByteBeat code", code, kMaxCodeSize);
            static bool success = true;
            static std::string compiled_text = "N/A";
            if (ImGui::Button("Compile"))
            {
                success = byte_beat.Parse(code, compiled_text);
            }
            if (success) ImGui::Text("SUCCESS: %s", compiled_text.c_str());
            else ImGui::Text("ERROR:", compiled_text.c_str());
        }
