	...
	for (int paperIdx = 0; paperIdx < NUM_PAPERS; paperIdx++)
	{
		...
		for (int py = 0; py <= PAPER_Y_TILING; py++)
		{
			for (int px = 0; px <= PAPER_X_TILING; px++)
			{
				...
				float displ0 = 0.5f + 1.0f * cosf(i * 7602.2234f + px * 2517.3234f);
				float displ1 = 0.5f + 1.0f * cosf(i * 3455.2523f + px * 8435.7234f);
...
void MovingPapers::update(float deltaTime, bool noMovement)
{
	...
	float t = 0.5f - 0.5f * cosf(innerMove * PI);
	t = 0.5f - 0.5f * cosf(t * PI);
		...
		for (int py = 0; py < PAPER_Y_TILING; py++)
		{
			for (int px = 0; px < PAPER_X_TILING; px++)
			{
				...
				else
				{
					...
					{
						paper[paperIdx].snippet[py][px].rpy[dim] += deltaTime*cosf(dim * 34.12380f + px * 20.1239f + py * 12.1234f + time * 0.5f);
				...
				if (doDetach)
				{
					...
					int timePos = ((py*2  + px * 58901 + paperIdx * 2391445) % 1471 + 20357) % 127 + py * 18 * doDetach;
						...
						else if (pass == 1 && !paper[paperID].snippet[py][px].attached)
						{
							...
							for (int yp = 0; yp < 6; yp++)
							{
								for (int xp = 0; xp < 6; xp++)
								{
									...
									pointPos[yp][xp][0] += 2.0f / PAPER_Y_TILING * nvec[0] * cosf(pointPos[yp][xp][0] / 2.0f * PAPER_Y_TILING * 0.75f + pointPos[yp][xp][1] / 2.0f * PAPER_Y_TILING * 0.75f);
									pointPos[yp][xp][1] += 2.0f / PAPER_Y_TILING * nvec[1] * cosf(pointPos[yp][xp][0] / 2.0f * PAPER_Y_TILING * 0.75f + pointPos[yp][xp][1] / 2.0f * PAPER_Y_TILING * 0.75f);
								}
							}
...
void MovingPapers::rotate(float pos[3], float rpy[3])
{
	...
	t[0] = pos[0];
	t[1] = cosf(rpy[0]) * pos[1] - sinf(rpy[0]) * pos[2];
	t[2] = sinf(rpy[0]) * pos[1] + cosf(rpy[0]) * pos[2];

	pos[0] = cosf(rpy[1]) * t[0] - sinf(rpy[1]) * t[2];
	pos[1] = t[1];
	pos[2] = sinf(rpy[1]) * t[0] + cosf(rpy[1]) * t[2];

	t[0] = cosf(rpy[2]) * pos[0] - sinf(rpy[2]) * pos[1];
	t[1] = sinf(rpy[2]) * pos[0] + cosf(rpy[2]) * pos[1];
	t[2] = pos[2];
