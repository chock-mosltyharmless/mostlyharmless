...
__inline void init_mzk_data()
{
	...
	// Generate noise
	seed = 1;
	for (samp = 0; samp < NOISE_LENGTH; samp++)
	{
		noise[samp] = frand() - 0.5f;
	}

	// filter the noise (very simple lowpass)
	for (multiplier = 0; multiplier < 2; multiplier++)
	for (samp = 1; samp < NOISE_LENGTH; samp++)
	{
