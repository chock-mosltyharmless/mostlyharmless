...
static int window_init( WININFO *info )
{
    ...
    {
	    // create window
		//info->hWnd = CreateWindow( "static",0,WS_POPUP|WS_VISIBLE|WS_MAXIMIZE,0,0,0,0,0,0,0,0);
		//HDC hDC = GetDC(hWnd);
...
int WINAPI WinMain( HINSTANCE instance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
...
#if 1
	BASS_Init(-1,44100,0,wininfo.hWnd,NULL);
	HSTREAM mp3Str=BASS_StreamCreateFile(FALSE,"GT-UnconditionalFroehlichsein.mp3",0,0,0);
    ...
    while (!done)
    {
		...
		int t = (timeGetTime() - startTime) * 441 / 10;
