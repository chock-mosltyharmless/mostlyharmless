...
/* Given a position, this function constructs the oh-so-famous Perlin		\n\
 * noise. */																\n\
float perlin2(vec3 position)													\n\
{																			\n\
	return (smooth_noise(position * .06125) - 0.5) * .5 +							\n\
		   (smooth_noise(position * .08) - 0.5) * .5 +	   						\n\
	       (smooth_noise(position * .123) - 0.5) * .4 +							\n\
		   (smooth_noise(position * .17) - 0.5) * .3 +							\n\
	       (smooth_noise(position * .25) - 0.5) * .2 +								\n\
		   (smooth_noise(position * .32) - 0.5) * .1;								\n\
}																			\n\
																			\n\
float getXSobel()															\n\
{																			\n\
...
{\n\
   ...
   \n\
   // movetable effect:\n\
   vec2 moveEffect = vec2(0.0);\n\
   float moveAmount = parameters[1][3];\n\
   float time = 3. * parameters[1][2];\n\
   float smNoise = perlin2(vec3(gl_FragCoord.xy * 0.25, time));\n\
   if (moveAmount > 0.01)\n\
   {\n\
		float dxNoise = perlin2(vec3(gl_FragCoord.xy * 0.25 + vec2(0.4, 0.0), time)) -\n\
						perlin2(vec3(gl_FragCoord.xy * 0.25 - vec2(0.4, 0.0), time));\n\
		float dyNoise = perlin2(vec3(gl_FragCoord.xy * 0.25 + vec2(0.0, 0.4), time)) -\n\
						perlin2(vec3(gl_FragCoord.xy * 0.25 - vec2(0.0, 0.4), time));\n\
		float moveAmount = parameters[1][3];\n\
		moveEffect = vec2(dyNoise, dxNoise) * sign(smNoise) * moveAmount;\n\
	}\n\
   \n\
   // standard readout: \n\
   vec4 inpt = 1.2 * texture2D(Texture0, gl_TexCoord[0].xy + moveEffect);\n\
   vec3 fgColor = inpt.rgb;\n\
   float alpha = sqrt(inpt.a);\n\
   float fgBW = dot(vec3(0.3, 0.59, 0.11), fgColor.rgb);\n\
   // do some magic with the move effect borders:\n\
   float brightener = smoothstep(1.0 - 0.1 * moveAmount, 1.0, 1.0 - abs(smNoise));\n\
   fgBW += 0.5 * smoothstep(0.0, 1.0, moveAmount) * pow(brightener, 10.0);\n\
   ...
   {\n\
	  ...
	  if (parameters[1][1] > 0.5) {\n\
	    fgData = fgData * vec3(1.0, 1.1, 1.2);\n\
