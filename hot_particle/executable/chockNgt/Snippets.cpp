...
void Snippets::draw(void)
{
	...
	for (int i = 0; i < NUM_SNIPPETS; i++)
	{
		float texX = 0.5f + 0.47f * cosf(i * 3784.5f);
		float texY = 0.5f + 0.47f * cosf(i * 6392.2f);

		// corners are; topleft, topright, bottomright, bottomleft
		float displ[4][2];
		for (int corner = 0; corner < 4; corner++)
		{
			displ[corner][0] = 0.5f + 1.0f * cosf(i * 7602.2f + corner * 2531.3f);
			displ[corner][1] = 0.5f + 1.0f * cosf(i * 3452.2f + corner * 8423.7f);
		}
		...
		for (int yp = 0; yp < 6; yp++)
		{
			for (int xp = 0; xp < 6; xp++)
			{
				...
				pointPos[yp][xp][0] += 0.5f * polyW * nvec[0] * cosf(pointPos[yp][xp][0] / polyW * 0.75f + pointPos[yp][xp][1] / polyW * 0.75f);
				pointPos[yp][xp][1] += 0.5f * polyW * nvec[1] * cosf(pointPos[yp][xp][0] / polyW * 0.75f + pointPos[yp][xp][1] / polyW * 0.75f);
			}
		}
