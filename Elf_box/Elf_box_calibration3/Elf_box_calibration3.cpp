...
struct Camera {
    ...
    void Update(bool update_background, bool refresh_accumulate) {
        ...
        for (int i = 0; i < width_ * height_; i++) {
            ...
            if (update_background) {
                // Does input * 2 so that it shoots dark to black
                background_average_[i] = kAlpha * background_average_[i] + (1.0f - kAlpha) * input * 2;
            }
    ...
    void SuppressAreas() {
        ...
        int *area_id = new int [width_ * height_];
        int *check_next = new int [width_ * height_ * 2 + 4];
        int current_area = 1; // Leave out one so that I can go -1 later
        for (int i = 0; i < width_ * height_; i++) area_id[i] = -1;
        ...
        for (int y_start = 0; y_start < height_; y_start++) {
            for (int x_start = 0; x_start < width_; x_start++) {
                ...
                if (area_id[start_index] < 0 && buffer_[start_index] >= check_brightness) {
                    ...
                    check_next[0] = start_index;
                    while (num_check_next > 0) {
                        num_check_next--;
                        ...
                        if (index >= 0 && index < width_ * height_ &&
                            ...
                            check_next[num_check_next + 3] = index - width_;
                            num_check_next += 4;
                        }
                    }

                    if (area_size >= SUPPRESS_MINIMUM_SIZE) {
                        int num_check_next = 1;
                        check_next[0] = start_index;
                        while (num_check_next > 0) {
                            num_check_next--;
