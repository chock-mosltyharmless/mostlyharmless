...
LPCSTR STRING_FROM_GUID(GUID Attr)
{
    ...
    //4cc: XXXXXXX-0000-0010-8000-00AA00389B71
    if ((Attr.Data2 == 0x0000) && (Attr.Data3 == 0x0010) &&
        (Attr.Data4[0] == 128) && (Attr.Data4[1] == 0) && (Attr.Data4[2] == 0) && (Attr.Data4[3] == 170) &&
        (Attr.Data4[4] == 0) && (Attr.Data4[5] == 56) && (Attr.Data4[6] == 155) && (Attr.Data4[7] == 113)) {
        *((DWORD *)(&(fourcc[0]))) = Attr.Data1;
        fourcc[4] = 0;
        pAttrStr = fourcc;
    }
...
std::string GetMediaTypeDescription(IMFMediaType * pMediaType)
{
    ...
    if (pszGuidStr != NULL)
    {
        description += std::string(pszGuidStr);
        description += ": ";
    }
    else
    {
    ...
    for (UINT32 i = 0; i < cAttrCount; i++)
    {
        ...
        case MF_ATTRIBUTE_GUID:
        {
            ...
            if (pValStr != NULL)
            {
                description += std::string(pValStr);
            }
            else
            {
                char buffer[500];
...
// List all installed camera names
void ListCameras(IMFActivate **videoDevices, UINT32 videoDeviceCount) {
    std::string description = "";
    HRESULT hr = NULL;
    WCHAR *webcamFriendlyName;

    for (unsigned int i = 0; i < videoDeviceCount; i++) {
        description += "Camera index: " + std::to_string(i) + ": ";
        hr = videoDevices[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME, &webcamFriendlyName, NULL);
        if (FAILED(hr)) description += "Could not get friendly name.";
        else {
            char buffer[500];
            wcstombs(buffer, webcamFriendlyName, 500);
            description += std::string(buffer) + "\n";
        }
        description += "\n";
    }

    MessageBox(NULL, description.c_str(), "Video camera names", MB_OK);
}
...
void ListModes(IMFSourceReader *pReader)
{
    ...
    while (SUCCEEDED(hr))
    {
        ...
        else if (SUCCEEDED(hr))
        {
            ...
            //printf("%s\n", GetMediaTypeDescription(pType).c_str());
            text += "Media Type index: " + std::to_string(dwMediaTypeIndex) + "    ";
            text += GetMediaTypeDescription(pType) + "\n";

            pType->Release();
        }
        ++dwMediaTypeIndex;

        if ((dwMediaTypeIndex & 15) == 0) {
            MessageBox(NULL, text.c_str(), "Video types", MB_OK);
            text = "";
        }
    }
