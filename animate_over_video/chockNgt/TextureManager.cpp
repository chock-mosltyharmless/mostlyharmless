...
int TextureManager::loadAVI(const char *filename, char *errorString)
{
    ...
    video_frame_index_[numVideos] = -1;
...
int TextureManager::GetVideoFrameID(const char *name, GLuint *id, char *errorString, int frame_index) {
    for (int i = 0; i < numVideos; i++) {
        if (strcmp(name, videoName[i]) == 0) {
            *id = videoTextureID[i];
            ...
            if (frame_index == video_frame_index_[i]) {
                // Correct frame is already in texture
                return 0;
            }

            while (video_frame_index_[i] < frame_index) {
                int frameFinished = 0;
                while (!frameFinished) {
                    ...
                    if (video_packet_[i].stream_index == video_stream_[i]) {
                        ...
                        if (frameFinished) {
                            ...
                            video_frame_index_[i]++;
                            if (video_frame_index_[i] == frame_index) {
