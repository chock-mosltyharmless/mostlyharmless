...
% lowpass and resample - This is terrible!s
%voiced = filter(hamming(4)./sum(hamming(4)), 1, voiced);
%unvoiced = filter(hamming(4)./sum(hamming(4)), 1, unvoiced);
%voiced = max(voiced(1:2:end-1, :), voiced(2:2:end, :));
%unvoiced = max(unvoiced(1:2:end-1, :), unvoiced(2:2:end, :));