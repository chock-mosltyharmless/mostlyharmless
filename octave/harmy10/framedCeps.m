...
% R = framedCeps(A, frameLength, frameStep)
%
% Calculate the framed cepstrum of the signal A
% with a given framelength and stepsize.
%
% The truth is that this does not calculate the
% cepstrum but rather the cross correlation.

  
  % add 0s to get whole frames. Whatever.
  if (frameLength > frameStep)
    A = [zeros(floor((frameLength-frameStep)./2), 1); ...
		 A; ...
		 zeros(ceil((frameLength-frameStep)./2), 1)];
  end
