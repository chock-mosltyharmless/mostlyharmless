...
% The length values
fprintf(fid, 'unsigned short fontLength[2536] = {\n');
oldValue = coreData(1);
position = 2;
length = 1;
numValues = 1;
...
fprintf(fid, '  %d};\n\n', length);
% The value values
fprintf(fid, 'unsigned char fontValues[2536] = {\n');
oldValue = coreData(1);
position = 2;
while (position < w*h*4)
	if (coreData(position) ~= oldValue)
...
if (false)
	fprintf(fid, 'unsigned char fontCompressed[4][72][480] = {\n');
	for color = 1:4
		fprintf(fid, '  {\n');
		for y = 1:w
			fprintf(fid, '    {');
