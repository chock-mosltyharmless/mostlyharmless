...
vec2 getFog(vec2 relPos)
{
	...
	fog += 0.2 * texture2D(Texture0, swirlypos.xy * 14.99);
	fog += 0.1 * texture2D(Texture0, swirlypos.xy * 24.99);
	fog = 0.5 * fog;
	...
	redfog -= 1.0 - parameters[0][3];
	blackness -= 1.0 - parameters[0][3];
...
void main(void)
{
	float time = parameters[0][0] * 0.15;
