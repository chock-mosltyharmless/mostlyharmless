...
float noiseData2[TM_NOISE_TEXTURE_SIZE * TM_NOISE_TEXTURE_SIZE * 4];
unsigned char noiseIntData2[TM_NOISE_TEXTURE_SIZE * TM_NOISE_TEXTURE_SIZE * 4];
...
int TextureManager::init(char *errorString)
{
	...
	createRenderTargetTexture(errorString, OFFSCREEN_WIDTH, OFFSCREEN_HEIGHT, TM_OFFSCREEN_NAME);
	createRenderTargetTexture(errorString, HIGHLIGHT_WIDTH, HIGHLIGHT_HEIGHT, TM_HIGHLIGHT_NAME);
...
int TextureManager::createNoiseTexture(char *errorString, const char *name)
{
	...
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA,
					  ...
					  GL_BGRA, GL_UNSIGNED_BYTE, noiseIntData2);
...
void TextureManager::makeIntTexture(void)
{
	...
	{
		if (noiseData2[i] < 0.0f)
		{
			noiseIntData2[i] = 0;
		}
		else if (noiseData2[i] >= 1.0f)
		{
			noiseIntData2[i] = 255;
		}
		else
		{
			noiseIntData2[i] = (unsigned char)(noiseData2[i] * 256.0f);
		}
	}
}
...
void TextureManager::generateNoiseTexture(void)
{
	...
	for (int y = 0; y < TM_NOISE_TEXTURE_SIZE * 2; y++)
	{
		for (int x = 0; x < TM_NOISE_TEXTURE_SIZE * 2; x++)
		{
			...
			largeNoiseData1[dstIdx++] = noiseData2[srcIdx++];
			largeNoiseData1[dstIdx++] = noiseData2[srcIdx++];
			largeNoiseData1[dstIdx++] = noiseData2[srcIdx++];
			largeNoiseData1[dstIdx++] = noiseData2[srcIdx++];
		}
	...
	for (int y = 0; y < TM_NOISE_TEXTURE_SIZE; y++)
	{
		for (int x = 0; x < TM_NOISE_TEXTURE_SIZE; x++)
		{
			...
			noiseData2[dstIdx++] = (float)largeNoiseData1[srcIdx++];
			noiseData2[dstIdx++] = (float)largeNoiseData1[srcIdx++];
			noiseData2[dstIdx++] = (float)largeNoiseData1[srcIdx++];
			noiseData2[dstIdx++] = (float)largeNoiseData1[srcIdx++];
		}
	...
	normalizeTexture(noiseData2, TM_NOISE_TEXTURE_SIZE * TM_NOISE_TEXTURE_SIZE);
	                          ...
	                          float rotation)
{
	...
	// bl
	glVertex3f(c*startX - s*endY + midX, s*startX + c*endY + midY, 0.5f);
	glTexCoord2f(1.0f, 1.0f);
	glVertex3f(c*endX - s*endY + midX, s*endX + c*endY + midY, 0.5f);
	glTexCoord2f(1.0f, 0.0f);
	glVertex3f(c*endX - s*startY + midX, s*endX + c*startY + midY, 0.5f);
	glTexCoord2f(0.0, 0.0f);
	glVertex3f(c*startX - s*startY + midX, s*startX + c*startY + midY, 0.5f);
	glEnd();
}
